...
/*   Updated: 2016/08/06 03:31:47 by avallete         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_print_elem(t_elem *elem, size_t max_len)
{
	if (elem->mode & IS_HOVERED)
		ft_bufferize(isatty(STDOUT_FILENO), tgetstr("us", NULL), 0);
	if (elem->mode & IS_SELECTED)
		ft_bufferize(isatty(STDOUT_FILENO), tgetstr("mr", NULL), 0);
	if (elem->color)
		ft_bufferize(isatty(STDOUT_FILENO), elem->color, 0);
	if ((elem->mode & (IS_HOVERED | IS_SELECTED)) != (IS_HOVERED | IS_SELECTED)\
			&& elem->len > max_len)
		ft_smarttrim(elem->name, max_len);
	else
		ft_bufferize(isatty(STDOUT_FILENO), elem->name, 0);
	if (elem->color)
		ft_bufferize(isatty(STDOUT_FILENO), "\033[m", 0);
	if (elem->mode & IS_HOVERED)
		ft_bufferize(isatty(STDOUT_FILENO), tgetstr("ue", NULL), 0);
	if (elem->mode & IS_SELECTED)
		ft_bufferize(isatty(STDOUT_FILENO), tgetstr("me", NULL), 0);
}
...
static void	ft_print_padded(t_elem *elem, int lpad, int rpad, size_t max_len)
{
	...
	ft_bufferize(isatty(STDOUT_FILENO), leftpad, 0);
	ft_print_elem(elem, max_len);
	ft_bufferize(isatty(STDOUT_FILENO), rightpad, 0);
}
...
void		ft_print_current_elem(t_select *env)
{
	...
	res = tgoto(cm, (env->page.relpos.x * env->page.columns_infos.column_len),\
			env->page.relpos.y);
	ft_bufferize(isatty(STDOUT_FILENO), res, 0);
